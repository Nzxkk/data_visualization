...
// 获取电子产品按省份统计的销售量数据
const fetchElectronicsSalesVolumeByProvince = async () => {
  try {
    const response = await axios.get('http://127.0.0.1:3006/api/ELECTRONICS_PROVINCE_SALES');
...
const renderChart = (data) => {
  ...
  const option = {
    backgroundColor: 'transparent', // 背景设置为深蓝半透明
    title: {
      text: '各省份电子产品销售量统计',
      left: 'center',
      textStyle: {
    ...
    visualMap: {
      ...
      inRange: {
        color: ['rgba(173, 216, 230, 0.8)', 'rgba(50, 107, 168, 0.8)'], // 浅蓝到深蓝的渐变透明
      },
      calculable: true,
    },
    series: [
...
// 启动定时轮询
const startPolling = () => {
  fetchElectronicsSalesVolumeByProvince(); // 首次加载数据
  intervalId = setInterval(() => {
    fetchElectronicsSalesVolumeByProvince(); // 每隔 5 秒获取数据
  }, 5000);
};
